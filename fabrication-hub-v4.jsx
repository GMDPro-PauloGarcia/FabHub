...
export default function FabHub() {
  ...
  useEffect(()=>{
    (async()=>{
      setSync("loading");
      try {
        const [d,p,e,i,j,sw] = await Promise.all([
          window.storage.get(KEYS.deals,true).catch(()=>null),
          window.storage.get(KEYS.projects,true).catch(()=>null),
          window.storage.get(KEYS.expenses,true).catch(()=>null),
          window.storage.get(KEYS.inflows,true).catch(()=>null),
          window.storage.get(KEYS.jos,true).catch(()=>null),
          window.storage.get(KEYS.swatches,true).catch(()=>null),
        ]);
        if(d) setDeals(JSON.parse(d.value));
        if(p) setProjs(JSON.parse(p.value));
        if(e) setExps(JSON.parse(e.value));
        if(i) setInfs(JSON.parse(i.value));
        if(j) setJos(JSON.parse(j.value));
        if(sw) setSwatches(JSON.parse(sw.value));
      } catch{}
  ...
  const save = useCallback(async(key,val)=>{
    setSync("saving");
    try{ await window.storage.set(key,JSON.stringify(val),true); setSync("saved"); }
    catch{ setSync("error"); }
  },[]);
