...
export const toolToSb  =(r)=>({id:r.id,name:r.name||'',borrower:r.borrower||'',borrowed_date:r.borrowedDate||null,expected_return:r.expectedReturn||null,actual_return:r.actualReturn||null,notes:r.notes||'',created_by:r.createdBy||''});
export const toolFromSb =(r)=>({...r,borrowedDate:r.borrowed_date||'',expectedReturn:r.expected_return||'',actualReturn:r.actual_return||'',createdBy:r.created_by});

export const drToSb    =(r)=>({id:r.id,dr_no:r.drNo||'',dr_date:r.drDate||r.date||null,supplier:r.supplier||'',project_id:(r.projectId==='__gmd_stocks__')?null:(r.projectId||null),project_name:r.projectName||'',po_number:r.poNumber||'',items:Array.isArray(r.items)?r.items:[],total:Number(r.total)||0,remarks:r.remarks||'',received_by:r.receivedBy||'',created_by:r.createdBy||''});
export const drFromSb  =(r)=>({...r,drNo:r.dr_no||'',drDate:r.dr_date||'',projectId:r.project_id,projectName:r.project_name||'',poNumber:r.po_number||'',items:r.items||[],total:Number(r.total)||0,receivedBy:r.received_by||'',createdBy:r.created_by});
